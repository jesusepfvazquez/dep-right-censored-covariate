---
title: "Robust Estimation with Outcome-Dependent Right-Censored Covariates"
...
- an auxiliary variable $A$ which represents age,
...
Naive complete-case analysis discards subjects with $D=0$, leading to:
...
- **AIPW**: augmented IPW estimator, combining IPW + augmentation  
- **AIPW-$\Lambda$**: a closed-form augmentation variant based on a $\Lambda$-type adjustment
...
- `W` = min(X, C)  
- `D` = I(X ≤ C)  
- `AX` = A - X; representing time to event
...
The **AIPW** estimator combines IPW with an augmentation term based on a model for $f_{X \mid Z}$, providing double-robustness-type behavior and improved efficiency.

In the package version, you supply:

- `model`: outcome model, e.g. `y ~ AW + Z`  
- `model_weights`: censoring model for $f_{C | (Y, Z)}$ 
- `gamma_x`: parameters of the $f_{X \mid Z}$ AFT model (coefficients + shape)  
- `model_xz`: covariate structure for $X | Z$ (RHS-only formula, e.g. `~ Z`)

In a typical workflow, you would:

1. Fit a working model for $X | Z$ to obtain $\hat\gamma_x$
2. Plug $\hat\gamma_x$into `estimate_beta_aipw()`.

Here, suppose we have a known or estimated `gamma_x` from the simulation:
...
Because the AIPW integrals can be numerically heavy, we keep these chunks as `eval = FALSE` in the vignette. Users can run them manually.
...
`vignettes/ODRCC-tutorial.Rmd`

2. Paste everything from the YAML header down into that file.

3. In R:

# ```{r, eval = FALSE}
# usethis::use_vignette("ODRCC-tutorial")  # if you haven't already
